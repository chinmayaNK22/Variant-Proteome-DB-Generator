### Extract Annotated Variants (V 1.0) from SnpEff output and store it in .txt file ###
### For any queries please contact chinnu.kemmaai@gmail.com ###
                ...
                try:
                    ...
                    if not 'synonymous_variant' in v_type:
                        ...
                        if a.split('|')[3] not in variant_locus:
                            variant_locus[a.split('|')[3]] = [raw_file + '@' + chrom + '@' + chrom_pos + '@' + ref_nucleo + '@' + alt_nucleo + '@' + v_type + '@' + a.split('|')[-6]]
                        else:
                            variant_locus[a.split('|')[3]].append(raw_file + '@' + chrom + '@' + chrom_pos + '@' + ref_nucleo + '@' + alt_nucleo + '@' + v_type + '@' + a.split('|')[-6]) 
        ...
        if k in feature:
            ...
            for j in v:
                try:
                    ...
                    op = j.split('@')[1] + '@' + j.split('@')[2] + '@' + j.split('@')[3] + '@' + j.split('@')[4] + '@' + k + '@' + ''.join(pro for pro in feature[k]) + '@' + j.split('@')[-2] + '@' + j.split('@')[-1] + '@' + output_variants
                    if op not in output:
                        output[op] = [j.split('@')[0]]
                    else:
        ...
        for i in file:
            ...
            if len(split_i) == 10:
                if split_i[4] != '.':
                    ...
                    c += 1
    print ('From file ' + vcf_file + ' ' + c + ' variants were extracted')

##file = 'ERR248976_ann.vcf'
##extract_variants(file)
##
##outfile = file.rstrip('.vcf') + '_variants.txt'
##with open(outfile, 'w') as outf:
##    outf.writelines('\t'.join(i) + '\n' for i in output)

#inpath = '.'
#infiles = []
#feature_table = '../Mavium_hominissuis/GCF_000007865.1_ASM786v1_feature_table.txt'
def extract_ann_vcf(inpath, feature_table):
    extract_feature_table(feature_table)
    if os.path.isfile(os.path.join(inpath)):
        if inpath.split('.')[-1] == 'vcf':
...
outfile = '{0}_annotation_info.txt'.format(args.infiles[0].rstrip('.txt'))
with open(outfile, 'w') as outf:
    outf.write('File\tNo. of Files\tChromosome\tChromosome Position\tReference Nucleotide\tAlternative Nucleotide\tLocus_tag\tProtein_Accession\tVariant_Type\tSNP(Three Letter)\tSNP(Single Letter)\n')
    outf.writelines('\t'.join(i) + '\n' for i in results)
